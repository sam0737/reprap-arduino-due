...
#define pexToggleortI(port, bits) \
  do { pexSysLockFromIsr; palToggleort(port, bits); pexSysUnlockFromIsr; } while (0)
...
#define pexEnableSigI(sig) \
  do { pexSysLockFromIsr; palEnableSig(sig); pexSysUnlockFromIsr; } while (0)
#define pexDisableSigI(sig) \
  do { pexSysLockFromIsr; palDisableSig(sig); pexSysUnlockFromIsr; } while (0)

#define palSetPinMode(p, mode) \
  palSetPadMode((p).port, (p).pin, mode)

#define palSetSigMode(sig, mode) \
  palSetPadMode((sig).pin.port, (sig).pin.pin, mode)
